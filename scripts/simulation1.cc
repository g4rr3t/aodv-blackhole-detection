/* Simulation 1: Test AODV protocol
 * 
 * Network topology
 * 
 *    n0 ------------> n1 ------------> n2 -------------> n3
 ...
 *
 * Scenario:
 * n0 will request a route to n3, which will be established
 * 
 * Output of this file:
 * 1. Generates sim1.routes file for routing table information and
 * 2. Sim1Animation.xml file for viewing animation in NetAnim.
 * 
 *
 *  Simulation pass criteria:
 *    Detector node does not detect black hole
 *    Flowmonitor reports 100% packets were transmitted from n0 to n3
 *
 *  Run script with command ./waf --run simulation1
 */
...
// ANIMATION SETTINGS ------------------------------------------------
  ...
  AnimationInterface anim ("Sim1Animation.xml"); // Mandatory
  AnimationInterface::SetConstantPosition (c.Get (0), 100, 550);
  AnimationInterface::SetConstantPosition (c.Get (1), 300, 550);
  AnimationInterface::SetConstantPosition (c.Get (2), 500, 550);
  AnimationInterface::SetConstantPosition (c.Get (3), 700, 550); 
  ...
  //increase size of detector and turn it white
  anim.UpdateNodeSize(detector.Get(0)->GetId(), 19.0, 19.0);
  anim.UpdateNodeColor(detector.Get(0), 255, 255 , 255);
  
  //setup route output
  Ptr<OutputStreamWrapper> routingStream = Create<OutputStreamWrapper> ("sim1.routes", std::ios::out);
...
//
  ...
  monitor->SerializeToXmlFile("simulation1.flowmon", true, true);
