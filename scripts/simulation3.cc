/* Simulation 3: Test VC Detection technique
 * 
 * Network topology
 *             
 *            n6 (black hole)
 ...
 *
 * Scenario:
 * n0 will send a RREQ to an address that does not exist.  n6 (black hole) will reply, detecting a black hole
 ...
 *    Black hole node is detected.  Feedback is printing to the screen when script is run
 *
...
// ANIMATION SETTINGS ------------------------------------------------
  ...
  AnimationInterface anim ("Sim3Animation.xml"); // Mandatory
  AnimationInterface::SetConstantPosition (c.Get (0), 100, 600);
  AnimationInterface::SetConstantPosition (c.Get (1), 300, 600);
  AnimationInterface::SetConstantPosition (c.Get (2), 500, 600);
  AnimationInterface::SetConstantPosition (c.Get (3), 700, 600); 
  ...
  //setup route output
  Ptr<OutputStreamWrapper> routingStream = Create<OutputStreamWrapper> ("sim3.routes", std::ios::out);
  ...
  for (std::map<FlowId, FlowMonitor::FlowStats>::const_iterator i = stats.begin (); i != stats.end (); ++i)
    {
      ...
      if ((t.sourceAddress=="10.1.2.1" && t.destinationAddress == "10.1.2.4"))
      {
          ...
          std::cout << "  Delivery %:   " << (float)i->second.rxBytes / (float)i->second.txBytes * 100.0 << "%\n";
      	  std::cout << "  Throughput: " << i->second.rxBytes * 8.0 / (i->second.timeLastRxPacket.GetSeconds() - i->second.timeFirstTxPacket.GetSeconds())/1024/1024  << " Mbps\n";
      }
     }

  monitor->SerializeToXmlFile("simulation3.flowmon", true, true);
